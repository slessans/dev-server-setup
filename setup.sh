...
# install exa, if not exists
if ! command -v exa &> /dev/null
then
    # check if apt contains a package for exa
    if apt-cache show exa &> /dev/null
    then
        echo "Installing exa from apt..."
        apt-get install -y --no-install-recommends exa
    else
        echo "Installing exa from prebuilt binaries..."
...
then
    if apt-cache show zoxide &> /dev/null
    then
        echo "Installing zoxide from apt..."
        apt-get install -y --no-install-recommends zoxide
    else
        echo "Installing zoxide from prebuilt binaries..."
...
sudo -u "$SUDO_USER" wget https://raw.githubusercontent.com/slessans/oh-my-zsh-sl-agnoster/main/sl-agnoster.zsh-theme -O "$USER_HOME/.oh-my-zsh/themes/sl-agnoster.zsh-theme"
sudo -u "$SUDO_USER" cp "$SCRIPT_DIR/.zshrc" "$USER_HOME/.zshrc"
